...
CenteredGridView {
    ...
    delegate: NavigableItemDelegate {
        ...
        Connections {
            ...
            function onStreamTweakStatusReceived(idx, status) {
                if (idx === index) {
                    streamTweakStatus = formatStreamTweakStatus(status)
                }
            }
        }
        ...
        Loader {
            ...
            sourceComponent: NavigableMenu {
                ...
                MenuSeparator {}
                MenuItem {
                    text: qsTr("Host NIC: %1").arg(streamTweakStatus.length > 0 ? streamTweakStatus : qsTr("checking..."))
                    enabled: false
                    visible: model.online && model.paired
                }
                NavigableMenuItem {
                    text: qsTr("Prepare StreamTweak")
                    onTriggered: {
                        computerModel.prepareStreamTweak(index)
        ...
        onPressAndHold: {
            computerModel.requestStreamTweakStatus(index)
            streamTweakStatus = qsTr("checking...")
    ...
    NavigableMessageDialog {
        ...
        text: qsTr("StreamTweak is preparing the host.\n\nYou can connect in %1 second%2.")
                  .arg(countdown)
                  .arg(countdown === 1 ? "" : "s")
    }
